...
function add_new_scan_pid_behav() {
	...
	sed 1d ${behav_proj_dir}/subjectlist.csv >> ${compiled_subj_files}/compiled_behav_sublist.csv;
...
function scanned_runs(){
	...
	raw_subj_fmri_dir=/oak/stanford/groups/menon/rawdata/scsnl/${pid}/visit${visit}/session${session}/fmri

	for run in `ls -d ${raw_subj_fmri_dir}/*`; do echo $(basename $run) >> ${project_dir}/data/subjectlist/run_list.txt; done
}
